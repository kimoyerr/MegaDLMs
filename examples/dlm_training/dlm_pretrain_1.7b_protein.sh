...
RUN_NAME=dlm_protein_training
train_data_prefix="$DATASETS_DIR/path/to/protein/train/data/prefix"
valid_data_prefix="$DATASETS_DIR/path/to/protein/valid/data/prefix"
TRAINING_TOKENS_PER_EPOCH=10000000000
EPOCHS=1000
GLOBAL_BATCH_SIZE=256
SEQ_LENGTH=1024 # proteins typically use shorter sequences than text
TOKENIZER=facebook/esm2_t33_650M_UR50D # ESM2 tokenizer from HuggingFace
...
GPT_MODEL_ARGS=(
    ...
    --swiglu
    --vocab-size 33 # ESM2 tokenizer has 33 tokens (20 amino acids + special tokens)
...
PLMT_ARGS=(
    ...
    --base-model vanilla
    --mask-token 32 # ESM2 mask token ID
...
EVAL_AND_LOGGING_ARGS=(
    ...
    --wandb-project dlm-protein
    --wandb-exp-name $RUN_NAME
    --wandb-run-id $RUN_NAME
    --wandb-save-dir ${LOGS_PATH}/${RUN_NAME}/wandb
    --wandb-mode $WANDB_MODE
    --wandb-tags 'dlm' 'protein' 'esm2'
    --wandb-notes 'protein language model training with ESM2 tokenizer'
    --wandb-resume allow
)
...
if [ "$CONVERT_CHECKPOINT_ONLY" == "convert_ckpt" ]; then
    ...
    if [ "$NODE_RANK" == "0" ]; then
        for CKPT_STEP_TO_CONVERT in "${ALL_CKPT_STEPS[@]}"; do
            ...
            MODEL_NAME="gptneox_1.7b_dlm_protein"
